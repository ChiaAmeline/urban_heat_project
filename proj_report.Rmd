...
This section performs an in-depth analysis of temperature data to uncover the trends and highlight significant patterns before moving to further deep analysis. Throughout the overall process we have covered:

*1. Yearly Trends in Average City Temperatures* : Line plots are used to visualize the changes in average city temperatures over time. This helps identify trends, such as global warming or cooling phases.

Key Findings : The time series plot of average city temperatures shows a clear upward trend, indicating a consistent rise in temperatures over the years. This aligns with global warming trends and highlights the growing impact of climate change.

```{r echo=FALSE, warning = FALSE, message = FALSE}
#Time series plot of average city temperature yearly
...
*2. Understanding Temperature Distributions* : Histograms are used to visualize the distribution of average temperatures at the country level (and optionally at the city level). This helps identify the spread and concentration of temperatures across different regions.

Key Findings : From the histograms, majority of countries and cities with high average temperatures fall within the range of 24 to 27°C. This indicates that regions with such temperature averages are most common in the dataset.
...
*3. Identifying Top 10 Hottest Countries* : The data is grouped by country, and the average temperature for each is calculated. The 10 hottest countries are identified based on their mean temperatures, highlighting regions with extreme heat.

Key Findings : The analysis identified the top 10 hottest countries by their average temperature, listed as:
Djibouti: 28.9°C ; Mali: 28.6°C ; Burkina Faso: 28.2°C ; Senegal: 28.1°C ; United Arab Emirates: 27.9°C ; Mauritania: 27.8°C ; Gambia: 27.7°C ; Niger: 27.6°C ; Benin: 27.3°C ; Chad: 27.2°C
These countries are geographically located in regions known for their hot climates, such as Africa and the Middle East.
...
*4. Bar Plot for Temperature Rankings* :A bar chart is created to showcase the top 10 countries by their average temperatures. The chart uses a color gradient from yellow to red to emphasize the varying intensity of heat, making the visualization intuitive and impactful.

Key Findings : The bar plot showcases these top 10 countries, with a heat gradient ranging from yellow (lower temperatures) to red (higher temperatures).This effectively emphasizes the intensity of heat in each country.
...
*5. Mapping Temperature Data* :A world map visualization highlights the top 10 hottest countries geographically. These countries are color-coded using a yellow-to-red gradient, while all other regions are displayed in gray for contrast. This map effectively combines spatial and statistical data to provide a clear global perspective.

Key Findings : The world map visually represents the top 10 hottest countries with vibrant yellow-to-red coloring, while other countries are shaded in gray. This clear contrast highlights the extreme temperatures of these regions.
Notably, the hottest countries are geographically close to the equator, aligning with the expectation of higher temperatures in equatorial regions due to consistent direct sunlight throughout the year.
...
*6. Global Heat Mapping* : A world map displays country-level average temperatures using a continuous color gradient, helping to understand the geographic distribution of heat across the globe.Cities are added to the map as points, with their sizes or colors representing their average temperatures. This layer enriches the visualization by connecting macro-level trends (countries) with micro-level data (cities).The addition of an equator line helps identify temperature patterns, emphasizing that the hottest regions are often located near the equator.

Key Findings : The equator's proximity plays a crucial role in shaping the temperature profile of the world's hottest regions.Cities with extreme temperatures are geographically clustered, reflecting localized climatic conditions within the broader regional trends.
...
For the temperature distribution, histogram reveals that most countries and cities have an average temperature range between 24°C and 27°C. This range represents the regions with the highest frequency of occurrences in the dataset, reflecting common temperature norms in warmer parts of the globe.

Countries such as Djibouti, Mali, Burkina Faso, and Senegal have consistently high average temperatures, exceeding 27°C. These regions are characterized by arid and semi-arid climates, often found in Africa and the Middle East.These temperatures align with expectations, as these countries experience extreme sunlight exposure and minimal rainfall

The highlighted map clearly shows that the hottest countries are concentrated near the equator. This finding supports the relationship between equatorial proximity and elevated temperatures due to consistent direct solar radiation throughout the year.

Regions with higher average temperatures are likely to face challenges such as heat-related health risks, water scarcity, and energy demand for cooling systems.These insights can aid in planning sustainable solutions such as heat-resistant infrastructure, water conservation strategies, and renewable energy initiatives.

The city-level data aligns with the country-level analysis, confirming that cities within the hottest countries also experience elevated average temperatures. This emphasizes the need for targeted interventions in urban areas of equatorial regions.

The time-series trend and increasing average temperatures over the years confirm the ongoing effects of global warming. This suggests the need for urgent climate adaptation and mitigation measures.

Cities in the hottest countries contribute to the upward trend in average temperatures, possibly due to the population or energy or other external factors

# STEP 03 - PREDICTIVE MODELLING

This section of the report presents predictive models to investigate the relationship between city temperatures with population, energy consumption and green area. The models aim to uncover how these factors interact and influence temperature. The analysis includes evaluating the significance of each predictor and comparing the accuracy of the models to discover insights.

## Prepare Data For Predictive Modelling

We first started off by joining the population, energy consumption and green area data into the temperature data using a left join operation, ensuring that each country's data was aligned by year. 
...
We conducted a correlation matrix analysis to examine the relationships between temperature, population, energy consumption, and green area. The correlation matrix provided a comprehensive overview of the strength and direction of the linear relationships between these variables, helping to identify significant predictors of temperature. 
...
We can observe that overall there are no strong correlation between the 3 factors and temperature, their correlation score are close to zero.

## Regression model

To further investigate the relationship between temperature and 3 factors, we uses both multiple regression and polynomial regression models. Multiple regression was used to model the relationship between the predictors and temperature, assuming a linear correlation between them. However, with the understanding that the correlation could be more complex, we extended the analysis with polynomial regression, which introduces non-linear terms to capture more complex relationships. 
...
cat(paste("R-squared:", round(summary(regression_model)$r.squared,3)), "\n")
cat(paste("Adjusted R-squared:", round(summary(regression_model)$adj.r.squared, 3)), "\n\n")

# Set seed for prediction test
set.seed(42)

# Randomly sample 5 rows from Cleaned_temp_data to test prediction
...
cat(paste("R-squared:", round(summary(poly_model)$r.squared,3)), "\n")
cat(paste("Adjusted R-squared:", round(summary(poly_model)$adj.r.squared, 3)), "\n")
...
random_data_for_predict <- random_data_for_predict %>%
  mutate(Multiple_regression_predictions = regression_predictions, Polynomial_regression_predictions = poly_predictions) 

random_data_for_predict_selected <- random_data_for_predict[, c("avg_temp", "Multiple_regression_predictions", "Polynomial_regression_predictions")]
knitr::kable(random_data_for_predict_selected, caption = "Regression Models Predictions")
```
We can see that populations and energy consumption have p-value lower than 0.05 suggesting significant effects on temperature. And although Multiple R-squared and Adjusted R-squared has improved from multiple regression model to polynomial regression model, they are still relatively low, only 7.45% and 27.6% of temperature in the data can be explained by the models. This suggest that there might be other factors that could better explain the temperature differences.

## Random Forest

Next, we applied random forest model to predict the relationship between temperature and the 3 factors. Random forest has the ability to handle complex, non-linear relationships and allowing us to understand how much each factor contributes to temperature variations. 
...
n <- nrow(test_data)
adj_rsq <- 1 - ((1 - rsq) * (n - 1)) / (n - 3 - 1)
cat("Adjusted R-squared:", round(adj_rsq,3), "\n")
...
```
We can observe that there is an improvement from the regression models, the R-squared values has improved. Given a R-squared of 0.578, around 58% of the temperature in the data can be explained by the model, capturing a good portion of the temperature variance based on the 3 factors. The value for importance of predictors indicate how much each predictor contributes to the predictive accuracy of the model. This model saw energy consumption being the most influential factor. When we plot them on a plot, we can see that there are points close to the line, suggesting minor differences between predicted and actual value.

**Analysis/Observation:** 


From all the 3 models conducted, we found out that energy consumption and population are key factors influencing temperature while green area shows weaker effects compared to the other two factors. Although random forest model provided the most robust predictions by explaining half of the data, a significant portion of the variability in temperature remains unexplained, indicating that other factors not included in the models could further enhance the predictions. 
...
High Density: Clusters in central Brazil around cities like Bras?lia and further south towards S?o Paulo and Rio de Janeiro.
